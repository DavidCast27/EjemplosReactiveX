...
import { switchMap, map, pluck, mergeAll } from 'rxjs/operators';
...
(() =>{
    ...
    const SW_PEOPLE =  'people/3/';                
    const getRequest = ( url: string ) => ajax.getJSON<any>(url);
    // ==================================================================

    // Realizar el llamado al URL para obtener a Luke Skywalker
    getRequest(`${SW_API}/${SW_PEOPLE}`).pipe(
        // Realizar los operadores respectivos aquí
        //primera respuesta
        // switchMap(person=>getRequest(person.species[0])),

        switchMap(person=>zip(of(person),getRequest(person.species[0]))),
